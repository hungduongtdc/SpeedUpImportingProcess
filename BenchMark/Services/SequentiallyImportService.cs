...
{
    public static class SequentiallyImportService
    {
        public static void SequentiallyImport(this string inputFileFullPath, TestDBContext context)
        {
            int count = 0;
            using CancellationTokenSource cancellationTokenSource = new CancellationTokenSource();
            inputFileFullPath
                .ReadAllLines<AttendanceCSVDto>(cancellationTokenSource.Token)
                .ForAll((val, index) =>
                {
#if DEBUG
                    if (index % 50000 == 0)
